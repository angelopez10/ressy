...
export interface ServiceDTO {
  ...
  sortOrder: number;
  staffIds: string[];
  /** Anticipo por servicio (migración 14). 'inherit' = default del negocio. */
  depositOverride: 'inherit' | 'none' | 'percent' | 'fixed';
...
export async function getServices(businessId: string): Promise<ServiceDTO[]> {
  ...
  const [{ data: services }, { data: links }] = await Promise.all([
    db
      .from('services')
      .select('id, name, description, duration_min, price_amount, buffer_before_min, buffer_after_min, is_active, sort_order, deposit_override, deposit_percent, deposit_amount')
  ...
  return (services ?? []).map((s) => ({
    ...
    depositOverride: s.deposit_override,
